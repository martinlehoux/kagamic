...
int test_json_int_array() {
    ...
    t_assert(ints[2] == 3);
    t_assert(sum == 6);
    t_assert_equal(Arena_get_used(perm), 384);
...
int test_json_empty_array() {
    ...
    t_assert_equal(Arena_get_used(perm), 136);
...
int test_json_nested_array() {
    ...
    t_assert_equal(Arena_get_used(perm), 363);
...
int test_json_empty_object() {
    ...
    t_assert_equal(Arena_get_used(perm), 296);
...
int test_json_object() {
    ...
    t_assert_equal(Arena_get_used(perm), 373);
    t_assert_equal(doc.object->len, 1);
    t_assert(Str_equals(*Vec_get(&doc.object->keys, Str, 0), S("key")));
    t_assert(Str_equals(*JSONObject_get(doc.object, S("key"))->string,
                        S("value"))) t_assert_equal(Arena_get_used(perm), 373);
